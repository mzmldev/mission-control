...
export default function AgentsPage() {
  ...
  const [agentSessionKey, setAgentSessionKey] = React.useState("");
  ...
  const handleCreateAgent = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createAgent({
        ...
        role: agentRole.trim(),
        status: agentStatus,
        sessionKey: agentSessionKey.trim() || undefined,
      });
    ...
    <Dialog open={isCreating} onOpenChange={handleDialogChange}>
      ...
      <DialogContent className="sm:max-w-[560px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Register a new agent and set its initial status.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleCreateAgent} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="h-10 w-full rounded-md border border-[#E8E4DB] bg-[#FDFCF8] px-3 text-sm text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#C75B39]/40"
              >
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setAgentSessionKey(e.target.value)}
                className="bg-[#FDFCF8] border-[#E8E4DB] text-sm"
              />
            </div>
          </div>
          ...
          <p className="text-xs text-[#8A8A82]">
            Session keys are optional and help link agents to runtime processes.
          </p>
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={!agentName.trim() || !agentRole.trim() || isSubmitting}
